...
const Navbar = () => {
  ...
  useEffect(() => {
    const changeNavbarColor = () => {
      ...
      let faqContactSectionPosition = 13580;
      ...
      if (window.scrollY >= faqContactSectionPosition) {
  ...
  return (
    <div className={`navbar ${navbarColor}`}>
      ...
      {/* Contact Box */}
      <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0 transition duration-200 ease-in-out transform hover:-translate-y-1 hover:scale-110 hover:text-red">
        <a href="#contact" class={`inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 ${textColor} whitespace-no-wrap bg-indigo-600 border border-indigo-700 rounded-sm shadow-sm hover:bg-indigo-700 focus:outline-none`}>
